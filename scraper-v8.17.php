...
/** فهرست برنچ‌های ریپو — برای دراپ‌داون */
if (isset($_GET['su_branches'])) {
    header('Content-Type: application/json; charset=UTF-8');
    $c = su_load();
    ...
    $r = su_http('https://api.github.com/repos/' . $repo . '/branches?per_page=100', $c['github_token'], true, 25);
    if (!$r['ok']) {
    ...
    $d = json_decode($r['body'], true);
    if (!is_array($d)) { echo json_encode(['ok' => false, 'error' => 'پاسخ نامعتبر']); exit; }
    $out = [];
    foreach ($d as $b) {
...
/** فهرست فایل‌های یک برنچ — فقط فایل‌های PHP برای انتخاب هدف */
if (isset($_GET['su_files'])) {
    header('Content-Type: application/json; charset=UTF-8');
    $c = su_load();
    $repo   = trim((string)($_GET['repo'] ?? $c['repo'] ?? ''));
    ...
    $r = su_http($u, $c['github_token'], true, 30);
    if (!$r['ok']) {
        echo json_encode(['ok' => false, 'error' => $r['code'] === 404 ? 'برنچ پیدا نشد' : $r['error']], JSON_UNESCAPED_UNICODE);
        exit;
    }
    $d = json_decode($r['body'], true);
    if (!isset($d['tree']) || !is_array($d['tree'])) { echo json_encode(['ok' => false, 'error' => 'درخت فایل دریافت نشد']); exit; }
    $files = [];
    foreach ($d['tree'] as $n) {
        if (($n['type'] ?? '') !== 'blob') continue;
        $p = (string)$n['path'];
        if (!preg_match('~\.php\d?$~i', $p)) continue;   // فقط فایل‌های PHP
        $files[] = ['path' => $p, 'size' => (int)($n['size'] ?? 0)];
    }
    // بزرگ‌ترها اول: فایل اصلی اسکریپر معمولاً همان است
    usort($files, fn($a, $b) => $b['size'] <=> $a['size']);
    echo json_encode(['ok' => true, 'files' => $files, 'truncated' => !empty($d['truncated'])], JSON_UNESCAPED_UNICODE);
    exit;
}
...
$detailSelectors=$profile['detailSelectors']??[];
...
$queue['entries'][]=['id'=>$queueId,'status'=>'running','profile_key'=>$profileKey,'url'=>$url,'profile_name'=>$profile['name']??$profileKey,'started_at'=>time(),'products_count'=>0];
extractWriteQueue($queue);

writeProgress(EXTRACT_PROGRESS_FILE,['running'=>true,'done'=>false,'total'=>0,'current'=>0,'started_at'=>$startedAt,'queue_id'=>$queueId,'recent_log'=>['⏳ شروع استخراج بک‌اند...'],'total_log_count'=>1,'extracted'=>0,'new'=>0,'price_changed'=>0,'removed'=>0,'unchanged'=>0,'url'=>$url,'profile_name'=>$profile['name']??$profileKey]);
...
writeProgress(EXTRACT_PROGRESS_FILE,['running'=>true,'done'=>false,'total'=>$maxPages,'current'=>$page,'started_at'=>$startedAt,'recent_log'=>$logs,'total_log_count'=>$page,'extracted'=>count($allProducts),'page'=>$page,'page_ok'=>true,'page_new'=>$newCount,'page_total'=>count($allProducts)]);
...
writeProgress(EXTRACT_PROGRESS_FILE,['running'=>true,'done'=>false,'total'=>$maxPages+$detailTotal,'current'=>$totalPages+$detailDone,'started_at'=>$startedAt,'recent_log'=>$logs,'total_log_count'=>$totalPages+$detailDone,'extracted'=>count($allProducts),'phase'=>'detail','detail_current'=>$detailDone,'detail_total'=>$detailTotal]);
}
usleep(200000);
}
}
...
$currPrice=$p['price']??'';
if(!$prev){
$newCount++;$newItems[]=['title'=>$p['title']??$p['name']??$key,'price'=>$currPrice,'key'=>$key];
}else{
$prevPrice=$prev['price']??'';
if($currPrice!==$prevPrice&&$currPrice!==''&&$prevPrice!==''){
$priceChanged++;$changedItems[]=['title'=>$p['title']??$p['name']??$key,'old_price'=>$prevPrice,'new_price'=>$currPrice,'key'=>$key];
}else{
$unchanged++;
}
}
...
$removedCount++;$removedItems[]=['title'=>$prev['title']??$prev['name']??$key,'price'=>$prev['price']??'','key'=>$key];
}
}
...
writeProgress(EXTRACT_PROGRESS_FILE,['running'=>false,'done'=>true,'total'=>$maxPages+$detailTotal,'current'=>$totalPages+$detailTotal,'started_at'=>$startedAt,'last_progress_ts'=>time(),'queue_id'=>$queueId,'recent_log'=>$finalLog,'total_log_count'=>$totalPages+$detailTotal+1,'extracted'=>count($allProducts),'new'=>$newCount,'price_changed'=>$priceChanged,'removed'=>$removedCount,'unchanged'=>$unchanged,'new_items'=>$newItems,'changed_items'=>$changedItems,'removed_items'=>$removedItems,'products_saved'=>true,'profile_key'=>$profileKey??profileKey($url),'total_pages'=>$totalPages]);

$queue=extractReadQueue();
foreach($queue['entries'] as &$qe){if($qe['id']===$queueId){$qe['status']='done';$qe['products_count']=count($allProducts);$qe['done_at']=time();$qe['new']=$newCount;$qe['price_changed']=$priceChanged;$qe['removed']=$removedCount;$qe['unchanged']=$unchanged;break;}}unset($qe);
extractWriteQueue($queue);

header('Content-Type: application/json; charset=UTF-8');
echo json_encode(['ok'=>true,'extracted'=>count($allProducts),'new'=>$newCount,'price_changed'=>$priceChanged,'removed'=>$removedCount,'unchanged'=>$unchanged,'new_items'=>$newItems,'changed_items'=>$changedItems,'removed_items'=>$removedItems,'products_saved'=>true,'profile_key'=>$profileKey??profileKey($url)],JSON_UNESCAPED_UNICODE);
exit;
}
...
// v8.19: Re-scrape using manual selectors before sending
cronRescrapeProfile($profile, $key, $profiles);
...
<div class="crow">
  <label>ریپو:</label>
  <input id="suRepo" placeholder="user/repo" dir="ltr" oninput="suDirty()" style="flex:1">
  <button class="btn btn-gray" onclick="suLoadBranches(true)" id="suBtnRepo" style="flex:0 0 auto;padding:8px 12px">🔄 بارگذاری</button>
</div>

<div class="crow su-pick">
  <label>برنچ:</label>
  <div style="flex:1;position:relative;min-width:150px">
    <input id="suBranch" placeholder="ابتدا ریپو را بارگذاری کنید" dir="ltr" autocomplete="off"
           oninput="suFilterBranch()" onfocus="suFilterBranch()">
    <div class="su-drop" id="suBranchDrop"></div>
  </div>
</div>
...
<div class="crow su-pick">
  ...
  <div style="flex:1;position:relative;min-width:150px">
    <input id="suPath" placeholder="ابتدا برنچ را انتخاب کنید" dir="ltr" autocomplete="off"
           oninput="suFilterFile()" onfocus="suFilterFile()">
    <div class="su-drop" id="suPathDrop"></div>
  </div>
</div>
<div style="font-size:10px;color:#64748b;margin:-4px 0 8px" id="suFileCount"></div>
            ...
            <div id="extractQueueSection" style="margin-top:10px">
                <div style="background:#1e293b;border:1px solid #475569;border-radius:10px;padding:12px">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                        ...
                        <button class="btn btn-gray" onclick="refreshExtractQueue()" style="font-size:10px;padding:3px 8px">🔄</button>
                    </div>
...
function startBackendSync(){
    ...
    fetch('?load_profile='+encodeURIComponent(url)).then(r=>r.json()).then(d=>{
        ...
        if(panel){
            ...
            panel.innerHTML='<div style="color:#a855f7;font-weight:bold;padding:8px;margin-bottom:4px;background:#2e106530;border-radius:6px">⚡ استخراج بک‌اند — پیشرفت زنده</div><div id="extractLog" style="max-height:400px;overflow-y:auto;font-size:11px;color:#e2e8f0"></div><div id="extractStats" style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:8px;margin-top:10px"></div><div style="text-align:center;margin-top:8px"><button class="btn btn-red" onclick="stopBackendExtract()">⏹ توقف</button></div>';
        }
...
// v7.81: Refresh extraction queue list
function refreshExtractQueue(){
    fetch('?extract_queue_status=1').then(r=>r.json()).then(d=>{
        ...
        if(!list)return;
        if(!entries.length){list.innerHTML='<div style="color:#64748b;font-size:11px;padding:4px">صف خالی — استخراج بک‌اند انجام نشده</div>';return;}
        let html='';
        entries.slice().reverse().forEach(e=>{
            const statusColor=e.status==='running'?'#a855f7':e.status==='done'?'#4ade80':'#f87171';
            const statusText=e.status==='running'?'⏳ در حال اجرا':e.status==='done'?'✅ تکمیل':'❌ خطا';
            const products=e.products_count||0;
            const newC=e.new||0;const changedC=e.price_changed||0;const removedC=e.removed||0;
            const time=e.started_at?new Date(e.started_at*1000).toLocaleString('fa-IR'):'';
            html+='<div onclick="showExtractLogModal(\''+esc(e.id)+'\')" style="padding:8px;border:1px solid #475569;border-radius:8px;margin-bottom:6px;cursor:pointer;background:#0f172a" title="کلیک برای مشاهده لاگ">';
            html+='<div style="display:flex;justify-content:space-between;align-items:center">';
            html+='<span style="color:'+statusColor+';font-weight:700;font-size:12px">'+statusText+'</span>';
            html+='<span style="color:#64748b;font-size:10px">'+time+'</span>';
            html+='</div>';
            html+='<div style="color:#e2e8f0;font-size:11px;margin-top:4px">'+esc(e.profile_name||e.url||'')+'</div>';
            if(products>0){
                html+='<div style="color:#94a3b8;font-size:10px;margin-top:2px">'+toFa(products)+' محصول';
                if(newC>0)html+=' | 🆕'+toFa(newC);
                if(changedC>0)html+=' | 💰'+toFa(changedC);
                if(removedC>0)html+=' | ❌'+toFa(removedC);
                html+='</div>';
            }
            html+='</div>';
        });
...
function showExtractLogModal(queueId){
    ...
    modal.innerHTML='<div style="background:#1e293b;border:1px solid #475569;border-radius:12px;padding:16px;width:90%;max-width:600px;max-height:80vh;overflow-y:auto;color:#e2e8f0">'
        +'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">'
        +'<span style="font-weight:700;font-size:14px;color:#a855f7">📋 لاگ استخراج بک‌اند</span>'
        +'<button class="btn btn-gray" onclick="closeExtractLogModal()" style="font-size:10px;padding:4px 8px">✕</button>'
        +'</div>'
        +'<div id="extractModalLog" style="font-size:11px;max-height:60vh;overflow-y:auto">بارگذاری...</div>'
        +'</div>';
    // Start polling for this entry's progress
    pollExtractLogModal();
}
...
function suCloseDrops() {
    ['suBranchDrop', 'suPathDrop'].forEach(id => {
        const el = $(id);
        if (el) el.classList.remove('open');
    });
}

/** فهرست برنچ‌ها را می‌گیرد. auto=true یعنی خودکار هنگام باز شدن، بدون پیام خطا. */
function suLoadBranches(manual) {
    const repo = $('suRepo').value.trim();
    if (!repo) { if (manual) showToast('ابتدا نام ریپو را وارد کنید', true); return Promise.resolve(); }

    const btn = $('suBtnRepo');
    if (btn) { btn.disabled = true; btn.textContent = '⏳'; }

    return fetch('?su_branches=1&repo=' + encodeURIComponent(repo))
        .then(r => r.json())
        .then(d => {
            if (!d.ok) {
                SU_BRANCHES = [];
                if (manual) showToast(d.error || 'دریافت برنچ‌ها ناموفق', true);
                return;
            }
            SU_BRANCHES = d.branches || [];
            if (manual) showToast('✓ ' + toFa(SU_BRANCHES.length) + ' برنچ یافت شد');
            $('suBranch').placeholder = 'کلیک کنید یا تایپ کنید';
            return suLoadFiles(false);
        })
        .catch(() => { if (manual) showToast('خطا در ارتباط', true); })
        .finally(() => { if (btn) { btn.disabled = false; btn.textContent = '🔄 بارگذاری'; } });
}

/** فهرست فایل‌های PHP برنچ انتخاب‌شده */
function suLoadFiles(manual) {
    const repo = $('suRepo').value.trim();
    const branch = $('suBranch').value.trim();
    if (!repo || !branch) return Promise.resolve();

    $('suFileCount').textContent = 'در حال دریافت فهرست فایل‌ها...';
    return fetch('?su_files=1&repo=' + encodeURIComponent(repo) + '&branch=' + encodeURIComponent(branch))
        .then(r => r.json())
        .then(d => {
            if (!d.ok) {
                SU_FILES = [];
                $('suFileCount').textContent = '';
                if (manual) showToast(d.error || 'دریافت فایل‌ها ناموفق', true);
                return;
            }
            SU_FILES = d.files || [];
            $('suFileCount').textContent = toFa(SU_FILES.length) + ' فایل PHP در این برنچ';
            $('suPath').placeholder = 'کلیک کنید یا تایپ کنید';
        })
        .catch(() => { $('suFileCount').textContent = ''; });
}

function suRenderDrop(dropId, items, onPick, emptyMsg) {
    const box = $(dropId);
    if (!box) return;
    if (!items.length) {
        box.innerHTML = '<div class="su-none">' + esc(emptyMsg || 'موردی یافت نشد') + '</div>';
        box.classList.add('open');
        return;
    }
    box.innerHTML = items.map((it, i) =>
        '<div class="su-opt" data-i="' + i + '"><span>' + esc(it.label) + '</span>' +
        '<span class="su-meta">' + esc(it.meta || '') + '</span></div>'
    ).join('');
    box.querySelectorAll('.su-opt').forEach(el => {
        el.onmousedown = e => { e.preventDefault(); onPick(items[+el.dataset.i]); };
    });
    box.classList.add('open');
}

function suFilterBranch() {
    const q = $('suBranch').value.trim().toLowerCase();
    if (!SU_BRANCHES.length) {
        suRenderDrop('suBranchDrop', [], () => {}, 'ابتدا دکمهٔ «بارگذاری» را بزنید');
        return;
    }
    const cur = SU ? SU.branch : '';
    const items = SU_BRANCHES
        .filter(b => b.name.toLowerCase().includes(q))
        .slice(0, 60)
        .map(b => ({ label: b.name, meta: (b.name === cur ? '● فعلی  ' : '') + b.sha, value: b.name }));
    suRenderDrop('suBranchDrop', items, it => {
        $('suBranch').value = it.value;
        suCloseDrops();
        suDirty();
        SU_FILES = [];
        $('suPath').placeholder = 'در حال بارگذاری...';
        suLoadFiles(true);
    });
}

function suFilterFile() {
    const q = $('suPath').value.trim().toLowerCase();
    if (!SU_FILES.length) {
        suRenderDrop('suPathDrop', [], () => {}, 'ابتدا ریپو و برنچ را انتخاب کنید');
        return;
    }
    const self = SU ? SU.self_name : '';
    const items = SU_FILES
        .filter(f => f.path.toLowerCase().includes(q))
        .slice(0, 80)
        .map(f => ({
            label: f.path,
            meta: (f.path.split('/').pop() === self ? '★ ' : '') + suFmtSize(f.size),
            value: f.path
        }));
    suRenderDrop('suPathDrop', items, it => {
        $('suPath').value = it.value;
        suCloseDrops();
        suDirty();
    });
}
...
document.addEventListener('click', e => {
    if (!e.target.closest('.su-pick')) suCloseDrops();
});
...
suLoad(() => {
    if (SU && SU.auto_check) setTimeout(suCheck, 1200);
    // فهرست‌ها را در پس‌زمینه آماده کن تا دراپ‌داون‌ها از همان ابتدا پر باشند
    if (SU && SU.repo) setTimeout(() => suLoadBranches(false), 2200);
});
...
function showLiveComparison(){
    ...
    fetch('?load_profile='+encodeURIComponent(url)).then(r=>r.json()).then(d=>{
        ...
        order.forEach(k=>{
            ...
            if(!prev){
                newCount++;newItems.push({title:curr.title||curr.name||'',price:currPrice});
            }else{
                const prevPrice=getFinalPriceNum(prev.price);
                if(currPrice!==prevPrice){
                    priceChanged++;changedItems.push({title:curr.title||curr.name||'',oldPrice:prevPrice,newPrice:currPrice});
                }else{unchanged++;}
            }
        });
        prevOrder.forEach(k=>{
...
function showExtractReport(type){
    ...
    if(type==='unchanged'){
        ...
        if(type==='changed'){html+='<th style="padding:6px;text-align:right">قیمت قبلی</th><th style="padding:6px;text-align:right">قیمت جدید</th>';}
        ...
        list.forEach((item,i)=>{
            ...
            if(type==='new'){html+='<td style="padding:4px;color:#4ade80">'+toFa(item.price)+' تومان</td>';}
            if(type==='changed'){html+='<td style="padding:4px;color:#f87171">'+toFa(item.oldPrice||item.old_price)+' تومان</td><td style="padding:4px;color:#4ade80">'+toFa(item.newPrice||item.new_price)+' تومان</td>';}
            if(type==='removed'){html+='<td style="padding:4px;color:#94a3b8">'+toFa(item.price)+' تومان</td>';}
            html+='</tr>';
        });
...
function showPhase2Modal(rejected,cats){
    ...
    h+='<div class="bsl-modal" style="width:800px">';
    h+='<div class="bsl-modal-head"><h2>🔄 Phase 2: اصلاح محصولات رد شده ('+toFa(rejected.length)+' محصول)</h2><button class="btn btn-gray" onclick="closePhase2()">✕</button></div>';
    h+='<div class="bsl-modal-body" style="max-height:70vh;overflow:auto">';
    // Auto-match attempt for each product
    rejected.forEach(p=>{
        const autoCatId=autoMatchBslCategoryJS(p.title,cats);
        h+='<div style="margin-bottom:10px;padding:10px;border:1px solid #f87171;border-radius:8px;background:#7f1d1d40" id="p2-'+p.id+'">';
        h+='<div style="font-weight:700;color:#f87171;margin-bottom:4px">🔴 '+esc(p.title)+' (ID#'+p.id+')</div>';
        h+='<div style="color:#94a3b8;font-size:11px;margin-bottom:6px">⚠️ '+esc(p.cat_reject_msg)+' | دسته فعلی: '+esc(p.current_cat_title||String(p.current_cat_id))+'</div>';
        h+='<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">';
        // Auto-match suggestion if found
        if(autoCatId>0){
            const autoCatName=cats.find(c=>c.id===autoCatId)?.name||'ID#'+autoCatId;
            h+='<button class="btn btn-green" onclick="bslFixCat('+p.id+','+autoCatId+','+autoCatId+')" id="btn-'+p.id+'">✅ دسته خودکار: '+esc(autoCatName)+' → ارسال</button>';
        }else{
            h+='<span style="color:#fbbf24;font-size:11px">⚠️ دسته خودکار یافت نشد — انتخاب دستی:</span>';
        }
        // v7.48: Manual category dropdown — searchable inline
        h+='<div style="position:relative;flex:1;max-width:250px">';
        h+='<input type="hidden" id="cat-'+p.id+'" value="0">';
        h+='<input type="text" id="catSearch-'+p.id+'" placeholder="جستجو دسته..." style="width:100%;padding:5px 8px;border:1px solid #475569;border-radius:6px;background:#0f172a;color:#e2e8f0;font-size:11px;direction:rtl" autocomplete="off">';
        h+='<div id="catList-'+p.id+'" style="display:none;position:absolute;top:100%;left:0;right:0;max-height:200px;overflow-y:auto;background:#1e293b;border:1px solid #475569;border-radius:6px;z-index:99999;direction:rtl"></div>';
        h+='</div>';
        h+='<button class="btn btn-orange" data-pid="'+p.id+'" onclick="bslFixCatManual(this)" id="btn-'+p.id+'-m" '+((autoCatId>0)?'style="opacity:0.7"':'')+'>🔄 ارسال مجدد دستی</button>';
    });
    h+='</div></div></div>';
    ...
    document.body.appendChild(div);
    // v7.48: Setup searchable dropdowns for each rejected product
...
function bslFixCat(productId,catId,autoCatId){
    ...
    fetch('?bsl_fix_cat=1&product_id='+productId+'&category_id='+useCatId).then(r=>r.json()).then(d=>{
        if(d&&d.ok){
            showToast('✓ دسته اصلاح شد: '+d.msg);
            const row=document.getElementById('p2-'+productId);
            if(row){row.style.borderColor='#22c55e';row.style.background='#14532d40';
                row.innerHTML+='<div style="color:#22c55e;font-weight:700;margin-top:6px">✅ اصلاح شد — دسته ID#'+useCatId+'</div>';
            }
        }else{
            showToast('❌ خطا: '+d?.error||'نامشخص',1);
            if(btn)btn.disabled=false;if(btnM)btnM.disabled=false;
            // Show full error
            const row=document.getElementById('p2-'+productId);
            if(row)row.innerHTML+='<div style="color:#f87171;margin-top:6px;font-size:11px">❌ '+esc(d?.error||'خطا')+': بازارسال ناموفق</div>';
        }

...
/**
 * v8.51: پیشنهاد دستهٔ آموخته‌شده برای چند عنوان یک‌جا.
 * ?catlearn_bulk=1&titles=<json آرایه‌ای از {id,title}>
 * فقط پیشنهاد می‌دهد؛ چیزی را تغییر نمی‌دهد.
 */
if (isset($_GET['catlearn_bulk'])) {
    header('Content-Type: application/json; charset=UTF-8');
    $raw = (string)($_POST['titles'] ?? $_GET['titles'] ?? '[]');
...
const CHANGELOG = [
  ...
  {v:'8.51', t:'اصلاح دستهٔ تأیید نشده‌ها با یادگیری و هوش مصنوعی', items:[
    ...
    'دکمهٔ «🧠 اعمال آموخته‌ها» همهٔ موارد قابل تشخیص را یک‌جا اصلاح می‌کند',
    'دکمهٔ «🤖 پیشنهاد هوش مصنوعی» برای هر محصول، و یک دکمه برای همه',
    'پیشنهاد هوش مصنوعی فقط نمایش داده می‌شود؛ اجرا با تأیید شماست',
    'هر تأیید — دستی یا هوش مصنوعی — به حافظهٔ یادگیری اضافه می‌شود',
    'نکته: پیشنهاد از هوش مصنوعی پیکربندی‌شدهٔ خودتان می‌آید، چون باسلام سرویس پیشنهاد دسته ندارد'
  ]},
  {v:'8.50', t:'رفع خطای دکمهٔ آموخته‌ها و شمارنده‌های قابل کلیک', items:[
    'رفع خطا: دکمهٔ «آموخته‌ها» خطای catLearnShow is not defined می‌داد — چهار تابع در ویرایش نسخهٔ ۸.۴۹ ناخواسته حذف شده بودند',
    'رفع باگ قدیمی: تابع bslInlineFixCat هرگز تعریف نشده بود و دکمهٔ «دستهٔ خودکار» در پنجرهٔ جزئیات همیشه خطا می‌داد',
    'ابزار بررسی خودکار اضافه شد که هر دکمهٔ بدون تابع را پیدا می‌کند — ۱۶۶ دکمه بررسی شد',
    'شمارنده‌های مغایرت‌گیری حالا قابل کلیک‌اند و فهرست محصولات را نشان می‌دهند',
    'فهرست یکسان‌ها هم نگه داشته می‌شود تا شمارندهٔ آن هم کار کند',
    'جستجو داخل فهرست، و نمایش وضعیت هر مورد پس از اعمال تغییرات'
  ]},
...
function reconReport(d,target){
  ...
  const cell=(icon,label,val,color,kind,enabled)=>
    ...
    +(enabled?' onclick="reconList(\''+kind+'\')" title="کلیک برای دیدن فهرست"':'')
    +'><b style="color:'+color+'">'+toFa(val)+'</b><span>'+icon+' '+label+'</span></div>';
  let h='<div style="background:#0f172a;border:1px solid #334155;border-radius:8px;padding:9px;font-size:11px">';
  h+='<div class="rc-grid">'
   +cell('📋','در پروفایل‌ها',nExp,'#e2e8f0','',false)
   +cell('🏪','در '+reconLabel(target),nRem,'#e2e8f0','',false)
...
/** v8.50: فهرست محصولات یک شمارنده */
function reconList(kind){
  ...
  if(!map){showToast('فهرستی نیست',1);return;}
  let h='<div class="bsl-modal-overlay" onclick="if(event.target===this)reconCloseList()">';
  h+='<div class="bsl-modal" style="max-width:760px">';
  h+='<div class="bsl-modal-head"><h2 style="color:'+map.color+'">'+map.title
    +' ('+toFa(map.total)+')</h2>'
    +'<button class="btn btn-gray" onclick="reconCloseList()">✕</button></div>';
  h+='<div style="padding:6px 12px;background:#111c31;border-bottom:1px solid #334155">'
    +'<input type="text" id="reconFilter" placeholder="جستجو در فهرست..." oninput="reconFilterRows()" '
    +'style="width:100%;padding:5px 8px;border:1px solid #475569;border-radius:6px;'
    +'background:#0f172a;color:#e2e8f0;font-size:11px;direction:rtl"></div>';
  h+='<div class="bsl-modal-body" style="max-height:60vh;overflow:auto">';
  if(!map.rows.length){
    h+='<div style="padding:24px;text-align:center;color:#64748b">موردی نیست</div>';
  }else{
    h+='<table class="bsl-modal-table"><thead><tr>'
      +'<th style="width:34px">#</th><th>عنوان</th>'
      +(kind==='diff'?'<th>فعلی</th><th>جدید</th>':'<th>قیمت</th>')
      +'<th style="width:52px">شناسه</th>'
      +(kind!=='matched'?'<th style="width:40px">وضعیت</th>':'')
      +'</tr></thead><tbody id="reconRows">';
    map.rows.forEach((it,n)=>{
      const money=v=>toFa(Number(v||0).toLocaleString('en-US'));
      h+='<tr class="rc-row" data-t="'+esc(String(it.title||'').toLowerCase())+'">'
        +'<td style="color:#64748b">'+toFa(n+1)+'</td>'
        +'<td>'+esc(String(it.title||''))+'</td>';
      h+=kind==='diff'
        ? '<td style="color:#94a3b8;white-space:nowrap">'+money(it.from)+'</td>'
          +'<td style="color:#e2e8f0;white-space:nowrap"><b>'+money(it.to)+'</b></td>'
        : '<td style="white-space:nowrap">'+money(it.price)+'</td>';
      h+='<td style="font-family:ui-monospace,monospace;color:#64748b">'+toFa(it.id||0)+'</td>';
      if(kind!=='matched'){
        h+='<td style="text-align:center">'+(it.done===true?'<span style="color:#4ade80">✓</span>'
          :(it.done===false?'<span style="color:#fca5a5">✗</span>':'<span style="color:#475569">—</span>'))+'</td>';
      }
      h+='</tr>';
    });
    h+='</tbody></table>';
    if(map.total>map.rows.length){
      h+='<div style="padding:8px;text-align:center;color:#64748b;font-size:10.5px">'
        +'نمایش '+toFa(map.rows.length)+' مورد از '+toFa(map.total)+'</div>';
    }
  }
  h+='</div></div></div>';
  const old=document.getElementById('reconListBox');if(old)old.remove();
  const div=document.createElement('div');div.id='reconListBox';div.innerHTML=h;
  document.body.appendChild(div);
}

function reconCloseList(){const m=document.getElementById('reconListBox');if(m)m.remove();}

function reconFilterRows(){
  const q=((document.getElementById('reconFilter')||{}).value||'').toLowerCase().trim();
  document.querySelectorAll('#reconRows .rc-row').forEach(tr=>{
...
function renderBslModal(products){
    ...
    if(isManageable&&products.length>0){
        ...
        // v8.51: در تب «تأیید نشده» دکمهٔ اصلاح دسته با مودال فاز ۲
        if(activeTab==='not_approved'){
            html+='<button class="btn btn-purple" style="font-size:11px;padding:4px 8px" '
                 +'onclick="bslPhase2Check()">\u{1F504} \u0627\u0635\u0644\u0627\u062D \u062F\u0633\u062A\u0647\u200C\u0628\u0646\u062F\u06CC (\u0641\u0627\u0632 \u06F2)</button>';
        }
        // Delete selected
...
/**
 * v8.50: این تابع هرگز تعریف نشده بود.
 *
 * دکمهٔ «دستهٔ خودکار» در پنجرهٔ جزئیات محصول و همچنین
 * bslInlineFixCatManual هر دو صدایش می‌زدند و هر بار
 * ReferenceError می‌داد. همان کار bslFixCat را می‌کند تا یادگیری
 * دسته‌بندی هم ثبت شود.
 */
function bslInlineFixCat(productId,catId){
    if(!catId||catId<=0){showToast('ابتدا دسته‌بندی انتخاب کنید!',1);return;}
    if(typeof bslFixCat==='function'){bslFixCat(productId,catId,catId);return;}
    // مسیر پشتیبان، اگر مودال فاز ۲ بارگذاری نشده باشد
    fetch('?bsl_fix_cat=1&product_id='+productId+'&category_id='+catId)
      .then(r=>r.json()).then(d=>{
        showToast(d&&d.ok?'✓ دسته اصلاح شد':('❌ '+((d&&d.error)||'خطا')),!(d&&d.ok));
      }).catch(()=>showToast('❌ خطا شبکه',1));
}
...
function showPhase2Modal(rejected,cats){
    ...
    h+='<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;padding:8px 14px;'
      ...
      +'<button class="btn btn-purple" onclick="p2ApplyLearned()" id="p2LearnBtn" '
      +'style="font-size:11px;padding:5px 10px" title="بر اساس دسته‌هایی که قبلاً دستی انتخاب کرده‌اید">'
    ...
    rows.forEach(r=>{
        ...
        // v8.51: پیشنهاد هوش مصنوعی — درخواستش جداگانه و با کلیک کاربر
...
/* =====================================================================
 *  v8.51: پیشنهاد هوش مصنوعی و اعمال آموخته‌ها در مودال فاز ۲
 * ===================================================================== */

/**
 * v8.51: بعد از هر انتخاب دستی، موارد باقی‌مانده با همان کلمهٔ اول را
 * پیدا می‌کند و روی کارتشان دکمهٔ «اعمال همین دسته» می‌گذارد.
 */
function p2SuggestSameWord(word){
  ...
  const fd=new FormData();
  fd.append('titles',JSON.stringify(rows.map(p=>({id:p.id,title:p.title||''}))));
  fetch('?catlearn_bulk=1',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
    const m=((d&&d.matches)||[]).filter(x=>x.word===word);
    if(!m.length)return;
    m.forEach(it=>{
      const box=document.getElementById('p2aiBox-'+it.id);
      if(!box||box.dataset.learned==='1')return;
      box.dataset.learned='1';
      box.innerHTML='<span style="color:#c4b5fd">🧠 مثل «'+esc(word)+'» → دستهٔ #'+it.cat_id+'</span>'
        +' <button class="btn btn-green" style="font-size:10px;padding:2px 8px;margin-right:6px" '
        +'onclick="bslFixCat('+it.id+','+it.cat_id+','+it.cat_id+')">✅ اعمال</button>';
    });
    const bar=document.getElementById('p2LearnBtn');
    if(bar)bar.textContent='🧠 اعمال آموخته‌ها ('+toFa(m.length)+')';
    showToast('🧠 '+toFa(m.length)+' محصول دیگر با «'+word+'» پیدا شد');
  }).catch(()=>{});
}

/** پیشنهاد هوش مصنوعی برای یک محصول */
function p2AskAi(pid){
  ...
  const btn=document.getElementById('p2ai-'+pid);
  if(btn){btn.disabled=true;btn.textContent='⏳ ...';}
  if(box)box.innerHTML='<span style="color:#93c5fd">🤖 در حال پرسیدن...</span>';
  return fetch('?bsl_ai_category=1&title='+encodeURIComponent(row.title||''))
    .then(r=>r.json()).then(d=>{
      if(btn){btn.disabled=false;btn.textContent='🤖 پیشنهاد هوش مصنوعی';}
      if(!box)return d;
      if(!d||!d.ok||!d.category_id){
        box.innerHTML='<span style="color:#fca5a5">🤖 '+esc((d&&d.error)||'پیشنهادی نداد')+'</span>';
        return d;
      }
      const nm=d.category_name||('#'+d.category_id);
      box.innerHTML='<span style="color:#67e8f9">🤖 پیشنهاد: <b>'+esc(nm)+'</b></span>'
        +' <button class="btn btn-green" style="font-size:10px;padding:2px 8px;margin-right:6px" '
        +'onclick="bslFixCat('+pid+','+d.category_id+','+d.category_id+')">✅ اجرا و ثبت</button>';
      return d;
    }).catch(()=>{
      if(btn){btn.disabled=false;btn.textContent='🤖 پیشنهاد هوش مصنوعی';}
...
function p2AskAiAll(){
  ...
  if(!rows.length){showToast('موردی باقی نمانده',1);return;}
  const btn=document.getElementById('p2AiAllBtn');
  if(btn){btn.disabled=true;btn.textContent='⏳ ...';}
  let i=0;
  (function next(){
    if(i>=rows.length){
      if(btn){btn.disabled=false;btn.textContent='🤖 پیشنهاد AI برای همه';}
      showToast('✓ پیشنهادها آماده شد — هرکدام را تأیید کنید');
      return;
    }
    const pid=rows[i++].id;
    if(btn)btn.textContent='⏳ '+toFa(i)+'/'+toFa(rows.length);
    Promise.resolve(p2AskAi(pid)).then(()=>setTimeout(next,900));
  })();
}

/**
 * v8.51: بقیه را بر اساس دسته‌هایی که قبلاً دستی انتخاب کرده‌اید اصلاح کن.
 * سرور فقط پیشنهاد می‌دهد؛ اعمال اینجا و با تأیید انجام می‌شود.
 */
function p2ApplyLearned(){
  const rows=(window._phase2Rows||[]).filter(p=>{
    const el=document.getElementById('p2-'+p.id);
    return el&&el.dataset.done!=='1';
  });
  if(!rows.length){showToast('موردی باقی نمانده',1);return;}
  const btn=document.getElementById('p2LearnBtn');
  if(btn){btn.disabled=true;btn.textContent='⏳ بررسی...';}
  const fd=new FormData();
  fd.append('titles',JSON.stringify(rows.map(p=>({id:p.id,title:p.title||''}))));
  fetch('?catlearn_bulk=1',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
    if(btn){btn.disabled=false;btn.textContent='🧠 اعمال آموخته‌ها';}
    const m=(d&&d.matches)||[];
    if(!m.length){
      showToast('برای هیچ‌کدام دستهٔ آموخته‌ای نیست — چند مورد را دستی اصلاح کنید',1);
      return;
    }
    if(!confirm('بر اساس انتخاب‌های قبلی شما، '+m.length+' محصول قابل اصلاح است.\n\nادامه؟'))return;
    if(btn){btn.disabled=true;}
    let i=0;
    (function next(){
      if(i>=m.length){
        if(btn){btn.disabled=false;btn.textContent='🧠 اعمال آموخته‌ها';}
        showToast('✓ '+toFa(m.length)+' محصول بر اساس آموخته‌ها اصلاح شد');
        return;
      }
      const it=m[i++];
      if(btn)btn.textContent='⏳ '+toFa(i)+'/'+toFa(m.length);
      const st=document.getElementById('p2st-'+it.id);
      if(st)st.innerHTML='<span style="color:#93c5fd">🧠 از «'+esc(it.word)+'»</span>';
      bslFixCat(it.id,it.cat_id,it.cat_id);

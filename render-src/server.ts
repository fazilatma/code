...
app.get('/api/destination/:target/products',async c=>{const target=c.req.param('target');if(!['woo','basalam'].includes(target))return c.json({ok:false,error:'Invalid target'},400);const all=await listDestinationProducts(target as any),q=String(c.req.query('q')||'').toLowerCase(),status=String(c.req.query('status')||''),image=String(c.req.query('image')||''),filtered=all.filter(x=>(!q||x.name.toLowerCase().includes(q)||String(x.id)===q)&&(!status||x.status===status)&&(!image||(image==='yes'?x.images.length>0:x.images.length===0))),limit=Math.min(200,Number(c.req.query('limit'))||50),offset=Math.max(0,Number(c.req.query('offset'))||0);return c.json({ok:true,total:filtered.length,items:filtered.slice(offset,offset+limit)})});
app.get('/api/destination/:target/products/:id',async c=>{const target=c.req.param('target');if(!['woo','basalam'].includes(target))return c.json({ok:false,error:'Invalid target'},400);const item=(await listDestinationProducts(target as any)).find(x=>x.id===Number(c.req.param('id')));return item?c.json({ok:true,item}):c.json({ok:false,error:'Product not found'},404)});

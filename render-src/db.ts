...
export async function migrate(): Promise<void> {
  await pool.query(`
    ...
    ALTER TABLE products ADD COLUMN IF NOT EXISTS image_valid boolean;
    ALTER TABLE products ADD COLUMN IF NOT EXISTS image_checked_at timestamptz;
    ALTER TABLE products ADD COLUMN IF NOT EXISTS image_error text;
...
export async function createJob(profileId:string,kind:Job['kind'],target:Job['target'],payload:Job['payload']={}):Promise<Job>{const client=await pool.connect();try{await client.query('BEGIN');await client.query('SELECT pg_advisory_xact_lock(hashtext($1))',[`${profileId}:${kind}`]);const active=await client.query(`SELECT * FROM jobs WHERE profile_id=$1 AND kind=$2 AND status IN('queued','running') ORDER BY created_at DESC LIMIT 1`,[profileId,kind]);if(active.rows[0]){let row=active.rows[0],needsFollowup=row.status==='running'&&Array.isArray(payload.keys)&&payload.keys.length>0;if(!needsFollowup){if(row.status==='queued'&&Array.isArray(payload.keys)&&payload.keys.length){const keys=[...new Set([...(row.payload?.keys||[]),...payload.keys])],merged={...row.payload,...payload,keys};const updated=await client.query('UPDATE jobs SET payload=$2,target=$3,updated_at=now() WHERE id=$1 RETURNING *',[row.id,JSON.stringify(merged),target]);row=updated.rows[0]}await client.query('COMMIT');return jobFromRow(row)}}const id=crypto.randomUUID(),maxAttempts=Math.max(1,Math.min(10,Number(process.env.JOB_MAX_ATTEMPTS)||3)),{rows}=await client.query(`INSERT INTO jobs(id,profile_id,kind,target,payload,max_attempts) VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,[id,profileId,kind,target,JSON.stringify(payload),maxAttempts]);await client.query('COMMIT');return jobFromRow(rows[0])}catch(error){await client.query('ROLLBACK');throw error}finally{client.release()}}
...
export async function updateLocalProduct(profileId:string,sourceKey:string,patch:Partial<Product>):Promise<Product|null>{const current=await getProduct(profileId,sourceKey);if(!current)return null;const allowed=['title','price','image','images','sku','brand','stock','weight','category','shortDesc','longDesc','variationGroups','variations','priceMin','priceMax'],next:any={...current};for(const key of allowed)if(Object.prototype.hasOwnProperty.call(patch,key))next[key]=(patch as any)[key];await upsertProduct(profileId,next);return next}
export async function archiveLocalProduct(profileId:string,sourceKey:string):Promise<boolean>{const client=await pool.connect();try{await client.query('BEGIN');const {rows}=await client.query('SELECT data,active FROM products WHERE profile_id=$1 AND source_key=$2 FOR UPDATE',[profileId,sourceKey]);if(!rows[0]){await client.query('ROLLBACK');return false}if(rows[0].active){await client.query('UPDATE products SET active=false,missing_since=COALESCE(missing_since,now()),updated_at=now() WHERE profile_id=$1 AND source_key=$2',[profileId,sourceKey]);await client.query('INSERT INTO product_changes(profile_id,source_key,change_type,old_data) VALUES($1,$2,$3,$4)',[profileId,sourceKey,'removed',JSON.stringify(rows[0].data)])}await client.query('COMMIT');return true}catch(error){await client.query('ROLLBACK');throw error}finally{client.release()}}
...
export async function changeReport(options:{hours?:number;profileId?:string;limit?:number}={}){const hours=Math.max(1,Math.min(24*365,Number(options.hours)||24)),limit=Math.max(1,Math.min(10000,Number(options.limit)||1000)),params:any[]=[hours];let where=`pc.created_at>=now()-make_interval(hours=>$1)`;if(options.profileId){params.push(options.profileId);where+=` AND pc.profile_id=$${params.length}`}params.push(limit);const {rows}=await pool.query(`SELECT pc.id,pc.job_id,pc.profile_id,p.data->>'name' profile_name,pc.source_key,pc.change_type,pc.old_data,pc.new_data,pc.created_at FROM product_changes pc LEFT JOIN profiles p ON p.id=pc.profile_id WHERE ${where} ORDER BY pc.created_at DESC LIMIT $${params.length}`,params),counts=rows.reduce((out:any,row:any)=>(out[row.change_type]=(out[row.change_type]||0)+1,out),{}),profiles:Record<string,any>={};for(const row of rows){profiles[row.profile_id]??={profileId:row.profile_id,name:row.profile_name||row.profile_id,added:0,updated:0,removed:0};profiles[row.profile_id][row.change_type]=(profiles[row.profile_id][row.change_type]||0)+1}return{hours,counts,profiles:Object.values(profiles),items:rows.map(row=>({...row,diff:productDiff(row.old_data,row.new_data)}))}}
export async function pruneProductChanges(days=90,maxRows=100000){const older=await pool.query(`DELETE FROM product_changes WHERE created_at<now()-make_interval(days=>$1)`,[Math.max(7,days)]),overflow=await pool.query(`DELETE FROM product_changes WHERE id IN(SELECT id FROM product_changes ORDER BY id DESC OFFSET $1)`,[Math.max(1000,maxRows)]);return{older:older.rowCount||0,overflow:overflow.rowCount||0}}
...
export async function setProductImageStatus(profileId:string,sourceKey:string,valid:boolean,error=''){await pool.query('UPDATE products SET image_valid=$3,image_checked_at=now(),image_error=$4 WHERE profile_id=$1 AND source_key=$2',[profileId,sourceKey,valid,error.slice(0,500)])}
export async function mediaStatus(profileId=''){const {rows}=await pool.query(`SELECT count(*)::int total,count(*) FILTER(WHERE image_valid=true)::int valid,count(*) FILTER(WHERE image_valid=false)::int invalid,count(*) FILTER(WHERE image_valid IS NULL)::int unchecked FROM (SELECT image_valid FROM products WHERE ($1='' OR profile_id=$1) AND active=true) x`,[profileId]);return rows[0]}
...
export async function restoreBackup(bundle: any): Promise<{ profiles: number; products: number; states: number }> {
  ...
  try {
    ...
    for (const row of bundle.products || []) { await client.query(`INSERT INTO products(profile_id,source_key,data,title,price,source_url,remote_woo_id,remote_basalam_id,created_at,updated_at,active,missing_since,image_valid,image_checked_at,image_error) VALUES($1,$2,$3,$4,$5,$6,$7,$8,COALESCE($9,now()),now(),COALESCE($10,true),$11,$12,$13,$14) ON CONFLICT(profile_id,source_key) DO UPDATE SET data=EXCLUDED.data,title=EXCLUDED.title,price=EXCLUDED.price,source_url=EXCLUDED.source_url,remote_woo_id=EXCLUDED.remote_woo_id,remote_basalam_id=EXCLUDED.remote_basalam_id,active=EXCLUDED.active,missing_since=EXCLUDED.missing_since,image_valid=EXCLUDED.image_valid,image_checked_at=EXCLUDED.image_checked_at,image_error=EXCLUDED.image_error,updated_at=now()`, [row.profile_id,row.source_key,row.data,row.title,row.price,row.source_url,row.remote_woo_id,row.remote_basalam_id,row.created_at,row.active,row.missing_since,row.image_valid,row.image_checked_at,row.image_error]); productCount++; }
...
function jobFromRow(row: any): Job {
  return { id: row.id, profileId: row.profile_id, kind: row.kind, target: row.target, payload:row.payload||{}, status: row.status, phase: row.phase,

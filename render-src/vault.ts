...
export function mergeConnections(base: ConnectionVault, input: any): ConnectionVault {
  ...
  const providers=Array.isArray(input?.ai?.providers)?input.ai.providers.map((p:any,i:number)=>{const rawModels=Array.isArray(p?.models)?p.models:[],models=rawModels.map((m:any)=>typeof m==='string'?m:String(m?.id||m?.name||'')).filter(Boolean),inferredReasoning=rawModels.filter((m:any)=>typeof m==='object'&&(m?.reasoning===true||m?.isReasoning===true||m?.features?.includes?.('reasoning'))).map((m:any)=>String(m?.id||m?.name||'')).filter(Boolean);return{id:text(p?.id)||`provider-${i+1}`,name:text(p?.name)||text(p?.id)||`Provider ${i+1}`,baseUrl:text(p?.baseUrl||p?.base_url).replace(/\/$/,''),apiKey:text(p?.apiKey||p?.api_key),models,reasoningModels:Array.isArray(p?.reasoningModels)?p.reasoningModels.map(String).filter((m:string)=>models.includes(m)):inferredReasoning,enabled:p?.enabled!==false,kind:['ollama','cloudflare'].includes(p?.kind)?p.kind:'openai'}}):base.ai.providers;

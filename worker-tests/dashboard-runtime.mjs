...
assert.match(script,/نسخهٔ ۱\.۱\.۰/,'the in-app change report identifies the current release');
...
try{(0,eval)(script+'\n;globalThis.__dashboardTest={runAiModelTests,connect,openDestinationCategoryManager,setDestinationTarget};')}catch(error){failures.push(error)}
...
const controlledFetch=async(input,init={})=>{const url=new URL(typeof input==='string'?input:input.url,base);if(url.pathname==='/api/ai/test-all'){const body=JSON.parse(String(init.body||'{}'));assert.equal(body.cursor,aiInvocations);assert.equal(body.categoryTitle,'ادو پرفیوم');aiInvocations++;const done=aiInvocations===3,results=Array.from({length:aiInvocations},(_,index)=>({ok:true,key:'runtime::model-'+index,provider:'runtime',providerName:'Runtime',model:'model-'+index,latencyMs:12+index,text:'پاسخ '+index,categoryTitle:body.categoryTitle,categoryResult:{ok:true,categoryId:100+index,categoryName:'دسته '+index,text:'{"category_id":'+(100+index)+'}',raw:{choice:100+index}},catResponse:'دسته '+index,raw:{usage:{total_tokens:7}}}));return new Response(JSON.stringify({ok:done,runId:'runtime-run',categoryTitle:body.categoryTitle,total:3,cursor:body.cursor,nextCursor:aiInvocations,done,batchSize:1,succeeded:aiInvocations,failed:0,categorySucceeded:aiInvocations,results,batchResults:[results.at(-1)]}),{headers:{'content-type':'application/json'}})}if(url.pathname==='/api/categories/basalam')return new Response(JSON.stringify({ok:true,items:[{id:100,name:'عطر زنانه',path:'عطر ← زنانه',leaf:true},{id:101,name:'عطر مردانه',path:'عطر ← مردانه',leaf:true}]}),{headers:{'content-type':'application/json'}});if(url.pathname==='/api/visual-ticket'){visualTickets++;return new Response(JSON.stringify({ok:true,ticket:'runtime-ticket'}),{headers:{'content-type':'application/json'}})}if(url.pathname==='/api/test-selector')return new Response(JSON.stringify({ok:true,count:2,values:['نمونهٔ اول','نمونهٔ دوم']}),{headers:{'content-type':'application/json'}});return browserFetch(input,init)};
...
assert.ok(document.querySelector('[data-ma="ai-test-all"]'),'AI all-model button is rendered');await globalThis.__dashboardTest.runAiModelTests(false);assert.equal(aiInvocations,3,'AI tests use three independent requests for three models');assert.match(document.querySelector('#resultModal .result-head b').textContent,/نتایج تست مدل‌های هوش مصنوعی/);assert.match(document.querySelector('#resultModal .result-body').textContent,/۳/);assert.match(document.querySelector('#resultModal .result-body').textContent,/ادو پرفیوم/);assert.equal(document.querySelectorAll('#resultModal [data-ai-filter]').length,3,'AI table has status/provider/search filters');assert.equal(document.querySelectorAll('#resultModal [data-ai-sort]').length,7,'every useful AI result column is sortable');assert.match(document.querySelector('#resultModal .result-table').textContent,/نتیجهٔ دسته‌بندی/);assert.match(document.querySelector('#resultModal .result-table').textContent,/دسته 0/);
const aiQuery=document.querySelector('#resultModal [data-ai-filter="query"]');aiQuery.value='model-2';document.getElementById('resultModal').oninput({target:aiQuery});assert.equal(document.querySelectorAll('#resultModal [data-ai-row]').length,1,'AI search filter narrows rows');assert.match(document.querySelector('#resultModal [data-ai-row]').textContent,/model-2/);
const resetQuery=document.querySelector('#resultModal [data-ai-filter="query"]');resetQuery.value='';document.getElementById('resultModal').oninput({target:resetQuery});let latencySort=document.querySelector('#resultModal [data-ai-sort="latency"]');document.getElementById('resultModal').onclick({target:latencySort});latencySort=document.querySelector('#resultModal [data-ai-sort="latency"]');document.getElementById('resultModal').onclick({target:latencySort});assert.match(document.querySelector('#resultModal [data-ai-row]').textContent,/model-2/,'second header click sorts descending');
const aiRow=document.querySelector('#resultModal [data-ai-row]');document.getElementById('resultModal').onclick({target:aiRow});assert.match(document.querySelector('#resultModal .result-head b').textContent,/جزئیات مدل/);assert.match(document.querySelector('#resultModal .result-body').textContent,/پاسخ خام دسته‌بندی/);assert.match(document.querySelector('#resultModal .result-body').textContent,/"choice": 102/);document.getElementById('resultModal').onclick({target:document.querySelector('#resultModal [data-modal-action="ai-table"]')});
globalThis.__dashboardTest.setDestinationTarget('basalam',false);await globalThis.__dashboardTest.openDestinationCategoryManager([{id:1,shopId:'55',title:'عطر اول'},{id:2,shopId:'55',title:'عطر دوم'}]);let categoryRoot=document.getElementById('resultModal');categoryRoot.onclick({target:categoryRoot.querySelector('[data-category-action="manual"]')});assert.equal(categoryRoot.querySelectorAll('[data-category-row]').length,2,'every category proposal can be corrected independently');let firstCategory=categoryRoot.querySelector('[data-category-row="0"]');firstCategory.value='101';categoryRoot.onchange({target:firstCategory});categoryRoot=document.getElementById('resultModal');assert.match(categoryRoot.querySelectorAll('.category-plan-row')[0].textContent,/عطر مردانه/);assert.match(categoryRoot.querySelectorAll('.category-plan-row')[0].textContent,/اصلاح دستی/);assert.match(categoryRoot.querySelectorAll('.category-plan-row')[1].textContent,/عطر زنانه/,'manual correction changes only its own product');
...
assert.equal(failures.length,0,failures.map(error=>error?.stack||String(error)).join('\n'));
console.log('dashboard runtime OK: remembered-profile restore/fallback, AI table interactions, per-product category correction, and detail controls');

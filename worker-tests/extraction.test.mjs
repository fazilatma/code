...
test('AI provider export is visibly confidential, round-trip capable, and Mistral catalog is chat-compatible only',async()=>{
  ...
  assert.deepEqual([...catalog.MISTRAL_CHAT_MODELS],[
    'mistral-medium-latest','mistral-small-latest','mistral-large-latest','zai-glm-5-2','voxtral-small-latest','codestral-latest','labs-leanstral-2603','ministral-3b-latest','ministral-8b-latest','ministral-14b-latest'
  ]);
  for(const incompatible of ['mistral-ocr-latest','voxtral-mini-latest','voxtral-mini-transcribe-realtime-2602','voxtral-mini-tts-latest','mistral-moderation-2603','codestral-embed','mistral-embed'])assert.doesNotMatch(catalogSource,new RegExp(`['\"]${incompatible}['\"]`),`${incompatible} requires a non-chat endpoint`);
  const ai={catalogVersion:0,providers:[{id:'mistral',name:'نام کاربر',baseUrl:'https://custom.example/mistral/v1',apiKey:'keep-this-key',models:['private-model'],enabled:true}]};assert.equal(catalog.upgradeAiProviderCatalog(ai),true);assert.equal(ai.providers.length,1);assert.equal(ai.providers[0].apiKey,'keep-this-key');assert.equal(ai.providers[0].baseUrl,'https://custom.example/mistral/v1');assert.equal(ai.providers[0].enabled,true);assert.ok(ai.providers[0].models.includes('private-model'));assert.ok(ai.providers[0].models.includes('mistral-medium-latest'));ai.providers[0].models=ai.providers[0].models.filter(model=>model!=='mistral-small-latest');assert.equal(catalog.upgradeAiProviderCatalog(ai),false);assert.equal(ai.providers[0].models.includes('mistral-small-latest'),false,'a user deletion remains stable after the one-time upgrade');
  assert.match(vault,/catalogVersion:num\(input\?\.ai\?\.catalogVersion/);assert.match(vault,/upgradeAiProviderCatalog\(result\.ai\)/);
});
...
test('destination modals, green AI filtering, live progress, and clickable queue event details are fully wired',async()=>{
  ...
  assert.doesNotMatch(source,/aiMenuResult/);assert.match(source,/function updateAiTestProgress\(/);assert.match(source,/\.filter\(row=>row\.ok===true\)/);assert.match(source,/در آخرین اجرای تست مدل‌ها هیچ مدل سبزی ثبت نشده است/);
...
test('durable background runners cover AI refresh recovery and every unapproved Basalam page',async()=>{
  ...
  for(const endpoint of ['/api/ai/test-runs','/api/ai/test-runs/current','/api/ai/test-runs/control','/api/destination/basalam/category-runs','/api/destination/basalam/category-runs/current','/api/destination/basalam/category-runs/control'])assert.match(app,new RegExp(endpoint.replaceAll('/','\\/')));
  assert.match(background,/type BackgroundRun=/);assert.match(background,/type RunStatus='queued'\|'running'\|'paused'\|'done'\|'failed'/);assert.match(background,/testModelBatch/);assert.match(background,/destinationCatalog\('basalam',[\s\S]*page:run\.page/);assert.match(background,/status:'3567'/);assert.match(background,/applyBasalamCategory/);assert.match(background,/recoverBackgroundRuns/);

...
| `cloudflare-worker.js` | Cloudflare Workers | فالبک کلودفلر (مثلاً proxy.fazilat-ma.workers.dev) |
...
## ☁️ فالبک ورکر کلودفلر

اگر اتصال مستقیم به مقصد **ناموفق** باشد (تایم‌اوت، قطعی اتصال، DNS و…)، درخواست به‌صورت خودکار از طریق ورکر کلودفلر رله می‌شود. پیش‌فرض، ورکر `https://proxy.fazilat-ma.workers.dev` است و کاملاً قابل تغییر:

### در proxy.php
```php
'fallback_proxy'      => 'https://proxy.fazilat-ma.workers.dev', // آدرس ورکر فالبک؛ خالی = غیرفعال
'fallback_key'        => '',                                      // کلید ورکر (اگر ورکر PROXY_KEY داشته باشد)
'fallback_on_statuses' => [],                                     // فالبک فقط روی خطای اتصال؛ برای 403/451 هم: [403, 451]
```

### در proxy.js
```bash
PROXY_FALLBACK='https://proxy.fazilat-ma.workers.dev' \
PROXY_FALLBACK_KEY='' \
PROXY_FALLBACK_ON_STATUSES='' \
node proxy.js
```

### رفتار فالبک
- ترتیب تلاش: **مستقیم ← پراکسی‌های بالادستی ← ورکر کلودفلر** (فقط وقتی همه شکست بخورند)
- کنترل‌هدرها (`X-Proxy-UA` و…) به ورکر منتقل می‌شوند تا مقصد همان هویت را ببیند
- اگر ورکر هم در دسترس نباشد، خطای اصلی اتصال برگردانده می‌شود
- جلوگیری از حلقه: اگر مقصد همان دامنهٔ ورکر باشد، فالبک استفاده نمی‌شود
- ورکر باید همان API پارامتر `?url=` را داشته باشد؛ کد آماده در **`cloudflare-worker.js`** است — کافیست آن را در Worker خود دیپلوی کنید (اختیاری: Secret با نام `PROXY_KEY`)
...
- **فالبک خودکار به ورکر کلودفلر** در صورت شکست اتصال به مقصد (پیش‌فرض قابل تغییر)
...
- ✅ فالبک کلودفلر: شکست اتصال → عبور از ورکر (متد، بدنه و کنترل‌هدرها منتقل می‌شوند)
- ✅ ترتیب تلاش: مستقیم ✗ ← بالادستی ✗ ← فالبک ✓ — عدم استفاده از فالبک وقتی اتصال مستقیم سالم است
- ✅ فالبک روی وضعیت پاسخ (`fallback_on_statuses`) — گارد ضدحلقه (فالبک = خود مقصد) — ارسال کلید فالبک
- ✅ فالبک غیرفعال → خطای اتصال اصلی برگردانده می‌شود

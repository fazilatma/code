...
app.get('/health',c=>c.json({ok:true,app:'scraper4-cloudflare',runtime:'cloudflare-workers',databaseReady:Boolean(c.env.DB),databaseError:c.env.DB?null:'D1 binding DB is missing',workerInWeb:Boolean(c.env.JOBS),authenticationRequired:false,version:c.env.WORKER_VERSION||'1.7.0',time:new Date().toISOString()}));
...
app.get('/api/version',c=>c.json({ok:true,version:c.env.WORKER_VERSION||'1.7.0',runtime:'cloudflare-workers',deployment:'wrangler versions deploy / wrangler rollback'}));

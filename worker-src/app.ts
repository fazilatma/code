...
app.get('/health',c=>c.json({ok:true,app:'scraper4-cloudflare',runtime:'cloudflare-workers',databaseReady:Boolean(c.env.DB),databaseError:c.env.DB?null:'D1 binding DB is missing',workerInWeb:Boolean(c.env.JOBS),authenticationRequired:false,version:c.env.WORKER_VERSION||'1.10.0',time:new Date().toISOString()}));
...
app.get('/api/version',c=>c.json({ok:true,version:c.env.WORKER_VERSION||'1.10.0',runtime:'cloudflare-workers',deployment:'wrangler versions deploy / wrangler rollback'}));
...
app.post('/api/queue-watchdog',async c=>{const b=await jsonBody(c);await recoverBackgroundRuns(promise=>c.executionCtx.waitUntil(promise));return c.json({ok:true,reaped:await reapStalledJobs(Number(b.minutes)||30),backgroundRecovered:true})});
...
export async function scheduledTasks(env:Env,waitUntil:(promise:Promise<unknown>)=>void):Promise<void>{configureEnv(env);await ensureSchema(env.DB);await reapStalledJobs(30);const due=await enqueueDueProfiles(),queued=(await listJobs(200)).filter(job=>job.status==='queued'),seen=new Set<string>();for(const job of [...due,...queued])if(!seen.has(job.id)){seen.add(job.id);await enqueueJob(job,waitUntil)}await recoverBackgroundRuns(waitUntil);waitUntil(automationTick())}

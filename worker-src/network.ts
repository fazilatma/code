...
export async function safeTextViaWorker(raw:string,workerUrl:string,maxBytes=8_000_000):Promise<{text:string;url:string;contentType:string}>{const target=assertPublicUrl(raw).href,base=workerUrl.trim();if(!base)throw new Error('برای اتصال غیرمستقیم، Worker URL را در تنظیمات روش اتصال وارد کنید.');const gateway=base.includes('{url}')?base.replace('{url}',encodeURIComponent(target)):base.replace(/\/$/,'')+'/'+target.replace(/^\//,'');const response=await safeFetch(gateway,{headers:{'x-target-url':target,accept:'text/html,application/xhtml+xml'}},maxBytes);if(!response.ok)throw new Error(`Indirect Worker HTTP ${response.status} for ${target}`);return{text:textDecoder.decode(await response.arrayBuffer()),url:target,contentType:response.headers.get('content-type')||''};}
